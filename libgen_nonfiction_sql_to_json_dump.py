...
def main():
    ...
    rename_cols = {'Title': 'title', 'Author': 'authors', 'IdentifierWODash': 'ISBN'}
    columns = ['title', 'Author', 'ASIN', 'IdentifierWODash', 'MD5']
    cursor.execute(f'select {",".join(columns)} from updated')
    with open('libgen_nonfiction.json', 'a') as f:
        for row in parse_cursor(cursor, columns):
            row['url'] = f'https://libgen.rs/book/index.php?md5={row["MD5"]}'
            del row["MD5"]
